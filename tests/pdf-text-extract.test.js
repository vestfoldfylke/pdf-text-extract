(async () => {
	...
	console.log(
		"Manual tests because jest dont work out of the box with pdfjs-dist, and I am lazy",
	);
	{
		console.log(
			"PATH. Example-pdf 1 (one page, one column, some text here and there with different styling",
		);
		...
		if (!lineCheck)
			throw new Error('Missing "SOME Larger TEXT" on page 1 line 5');
		...
		if (!pdfData.pages[0].textItems[5].str)
			throw new Error("Missing texItem on page 1, item 5");
	}

	{
		console.log(
			"PATH. Example-pdf 2 (two pages, two columns, some text here and there with different styling, even some vertical text",
		);
		const pdfData = await pdfTextExtract({
		...
		const lineCheck =
			pdfData.pages[1].textLines[1] ===
			"maybe not if the text is really malformed";
		console.log("\t Check that lines are in place");
		if (!lineCheck)
			throw new Error(
				'Missing "maybe not if the text is really malformed" on page 2 line 2',
			);
		console.log("\t Check that metadata is present");
		if (!pdfData.metadata) throw new Error("Missing metadata");
		console.log("\t Check that styles are present");
		if (!pdfData.styles) throw new Error("Missing styles");
		console.log("\t Check that textItems are present");
		if (!pdfData.pages[0].textItems[3].str)
			throw new Error("Missing texItem on page 1, item 4");
	}

	{
		console.log(
			"UINT8ARRAY. Example-pdf 2 (two pages, two columns, some text here and there with different styling, even some vertical text",
		);
